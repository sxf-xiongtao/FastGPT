...
const AuditTable = () => {
  ...
  const [auditDetail, setAuditDetail] = useState<OperationListItemType>();
  ...
  return (
    <BoxCard display={'flex'} flexDirection={'column'} h={'100%'}>
      <Flex justify={'flex-end'} align={'center'} pb={'1rem'} gap={2} wrap="wrap" mr="2px">
        <HStack pb={4}>
          {isPc && <Box className="text-2xl font-bold text-[#405169]">审计记录表</Box>}
        </HStack>
...
function AuditDetailModal({ log, onClose }: { log: OperationListItemType; onClose: () => void }) {
  const { t } = useTranslation();
  const i18nData = adminAuditLogMap[log.event as AdminAuditEventEnum];
...
export async function getServerSideProps(content: any) {
  return {
    props: {
      ...(await serviceSideProps(content, ['account_team', 'common']))
    }
  };
}
