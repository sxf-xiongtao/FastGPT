...
const authOutLinkAppAccess = async ({
  ...
  req: NodeHttpRequest;
  outLinkConfig: Pick<ShareOutLinkSchemaType<any>, 'allowAnonymous' | 'appId'>;
}) => {
...
export const authOutLink = async ({
  ...
  await authOutLinkAppAccess({ req, outLinkConfig: result.outLinkConfig });
...
export async function authOutLinkChatStart({
  ...
  await authOutLinkAppAccess({ req, outLinkConfig });
