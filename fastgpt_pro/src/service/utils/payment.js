...
export class Payment {
    ...
    this.urls = {
      ...
      tradebill: ({ queryParams }) => {
        ...
        return {
          ...
          }${tar_type ? '&tar_type=' + tar_type : ''}`,
          method: `GET`,
          pathname: `/v3/bill/tradebill?bill_date=${bill_date}${
            bill_type ? '&bill_type=' + bill_type : ''
          }${tar_type ? '&tar_type=' + tar_type : ''}`
        };
      },
      fundflowbill: ({ queryParams }) => {
        ...
        return {
          ...
          method: `GET`,
          pathname: `/v3/bill/fundflowbill?bill_date=${bill_date}${
            account_type ? '&account_type=' + account_type : ''
          }${tar_type ? '&tar_type=' + tar_type : ''}`
        };
      },
  ...
  init() {
    return this.decodeCertificates();
  }
  ...
  async run({ pathParams, queryParams, bodyParams, type }) {
    ...
    let bodyParamsStr =
      bodyParams && Object.keys(bodyParams).length ? JSON.stringify(bodyParams) : '';
    let signature = this.rsaSign(
      `${method}\n${pathname}\n${timestamp}\n${onece_str}\n${bodyParamsStr}\n`,
      this.private_key,
      'SHA256withRSA'
    );
  ...
  async decodeCertificates() {
    ...
    if (result.status != 200) {
      return Promise.reject('获取证书列表失败');
    }
    let certificates =
      typeof result.data == 'string' ? JSON.parse(result.data).data : result.data.data;
  ...
  async verifySign({ timestamp, nonce, serial, body, signature }, repeatVerify = true) {
    ...
    if (!verifySerialNoPass && repeatVerify) {
      ...
      return Promise.reject('平台证书序列号不相符');
    }
  }
